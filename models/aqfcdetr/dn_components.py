...
def prepare_for_cdn(dn_args, training, num_queries, num_classes, hidden_dim, label_enc):
    """
        ...
        :return:
        """
    if not training or dn_args[1] <= 0:
        return None, None, None, None
    if training:
        targets, dn_number, label_noise_ratio, box_noise_scale = dn_args
        if targets is None:
            raise ValueError('DN training requires targets')
        if not targets or not any(len(target['labels']) for target in targets):
            return None, None, None, None
        ...
        map_known_indice = torch.empty(0, device=device, dtype=torch.long)
        if len(known_num):
            map_known_indice = torch.cat([torch.arange(int(num), device=device) for num in known_num])
