...
class AQFCDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets: List = None):
        ...
        (hs, reference, hs_enc, ref_enc, init_box_proposal, dn_meta,
         allocator_outputs, query_counts, query_valid_mask) = self.transformer(
        ...
        if self.aux_loss:
            ...
            for aux_output in out['aux_outputs']:
                aux_output['query_valid_mask'] = query_valid_mask
        ...
        out['dn_meta'] = dn_meta
        out['allocator_outputs'] = allocator_outputs
        return out
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        probability = src_logits.sigmoid()
        focal = F.binary_cross_entropy_with_logits(
            src_logits, target_classes_onehot, reduction='none')
        p_t = probability * target_classes_onehot + (1 - probability) * (1 - target_classes_onehot)
        focal = focal * ((1 - p_t) ** 2)
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        predicted_foreground = pred_logits.sigmoid().max(-1).values > 0.5
        valid_mask = outputs.get('query_valid_mask')
        if valid_mask is not None:
            predicted_foreground = predicted_foreground & valid_mask
        card_pred = predicted_foreground.sum(1)
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, target_num=None, not_to_xyxy=False, test=False):
        ...
        valid_masks = outputs.get(
            'query_valid_mask',
            torch.ones(out_logits.shape[:2], dtype=torch.bool, device=out_logits.device))
        executed_counts = outputs.get('executed_query_counts', valid_masks.sum(1))
        all_boxes = out_bbox if not_to_xyxy else box_ops.box_cxcywh_to_xyxy(out_bbox)
        results = []
        for batch_index in range(out_logits.shape[0]):
            ...
            image_height, image_width = target_sizes[batch_index]
            scale = torch.stack([image_width, image_height, image_width, image_height])
            boxes = boxes * scale
            if self.nms_iou_threshold > 0:
                keep = nms(boxes, scores, iou_threshold=self.nms_iou_threshold)
                scores, labels, boxes = scores[keep], labels[keep], boxes[keep]
            results.append({
                'scores': scores, 'labels': labels, 'boxes': boxes,
                'executed_query_count': int(executed_counts[batch_index].item())})
        return results


@MODULE_BUILD_FUNCS.registe_with_name(module_name='aqfcdetr')
def build_aqfcdetr(args):
    ...
    model = AQFCDETR(
        ...
        aux_loss=True, iter_update=True, query_dim=4,
    ...
    if args.use_dn:
        ...
        if 'loss_nwd' in weight_dict: weight_dict['loss_nwd_dn'] = weight_dict['loss_nwd']
    ...
    if args.two_stage_type != 'no':
        ...
        _coeff_weight_dict = {
            'loss_ce': 1.0, 'loss_bbox': 1.0 if not no_interm_box_loss else 0.0,
