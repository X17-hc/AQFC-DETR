...
class MultiheadAttention(Module):
    ...
    """
    # 这个实现是基于 PyTorch 官方的 torch.nn.MultiheadAttention 模块修改而来的，
    # 主要目的是支持 Query (Q), Key (K), 和 Value (V) 可以有不同的特征维度（embed_dim, kdim, vdim），
    # 并且移除了 Q, K, V 的输入投影层（即假设输入 $Q, K, V$ 已经是投影后的或不需要再投影）。
    bias_k: Optional[torch.Tensor]
    bias_v: Optional[torch.Tensor]

    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None):
        super(MultiheadAttention, self).__init__()
        # 1. Query，Key，Value 的维度设置
        self.embed_dim = embed_dim
        self.kdim = kdim if kdim is not None else embed_dim
        self.vdim = vdim if vdim is not None else embed_dim
        # 检查 Q, K, V 是否使用相同的维度。这会影响 forward 中的调用路径
        self._qkv_same_embed_dim = self.kdim == embed_dim and self.vdim == embed_dim

        # 2. 头数和维度检查
        self.num_heads = num_heads
        self.dropout = dropout
        self.head_dim = embed_dim // num_heads  # 每个注意力头的 Q, K 维度
        ...
        self.bias_k = self.bias_v = None  # Key/Value 序列偏置
        self.q_proj_weight = None
        self.k_proj_weight = None
        self.v_proj_weight = None

        self.add_zero_attn = add_zero_attn  # 是否添加一个零向量作为额外的 Key/Value 对
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            # 如果 Q, K, V 维度不同，使用单独的 Q/K/V 权重 (尽管这里是 None)
            ...
            # 如果 Q, K, V 维度相同，使用统一的权重路径 (尽管这里是 None)
            return multi_head_attention_forward(
                query, key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                self.bias_k, self.bias_v, self.add_zero_attn,
...
def multi_head_attention_forward(query: Tensor,
    ...
    # 核心修改点: 假设输入 $Q, K, V$ 已经是投影后的结果，直接进行缩放
    q = query * scaling
    k = key
    v = value
    ...
    # 4. 分割多头：准备Q, K, V 进行 Batch Matrix Multiplication (BMM)
    # 形状变化: (Length, Batch, Dim) -> (Batch*Heads, Length, Head_Dim)
    ...
    # 5. 处理零注意力。在 K 和 V 序列的末尾添加一个零向量，并更新 src_len 和 mask， 然后拼接
    if add_zero_attn:
        src_len += 1
    ...
    # 9. Softmax  这里的 Softmax 实现是为了数值稳定性，先减去最大值再 Softmax
