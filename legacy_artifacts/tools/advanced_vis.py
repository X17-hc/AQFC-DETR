...
import json
import os
from torchvision.ops import nms  # ★ 新增：用于彻底消除重复预测框和标注
...
# 4. 主可视化函数
# ──────────────────────────────────────────────────────────────────────────────
def run_advanced_vis(img_path, gt_boxes, model, device,
...
if __name__ == "__main__":
    ...
    OUTPUT_FOLDER = "/workspace/DQDetr/logs/vis_advanced"

    SCORE_THR = 0.4
    IOU_THR = 0.5
    PRED_NMS_THR = 0.5  # 预测框去重阈值 (越小去重越狠，如果是密集拥挤目标可调高到 0.6)
    ...
    for idx, img_filename in enumerate(image_files, start=1):
        ...
        res_img = run_advanced_vis(img_path, gt_boxes, model, device,
